...
def ginterpolate (im, X, Y=None, output_type=None, order=1, 
    ...
    """
      Emulate the IDL interface to the interpolate
    ...
    if dim == 1:
       if szx == 1:
           ...
           z = nd.map_coordinates(im, X, order=order, mode=mode, cval=cval)
       elif szx == 2:
           lim = np.size(im)
           limy,limx = lim,lim
           if Y == None: 
       ...
       if szx == 1 and Y != None:
           z = nd.map_coordinates(im, [X,Y], order=order, mode=mode, cval=cval)
       elif szx == 2:
           if Y == None: Y = X
           z = nd.map_coordinates(im,[X,Y], order=order, mode=mode, cval=cval)
    else:
