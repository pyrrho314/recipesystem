...
def gauss2d(stampArray,pixelscale,initialParams,stampPars, outFile, debug, frame):
    ...
    # Using a try/catch to take care of a difference in the call format to 
    # minpack.leastsq().  Prior to scipy version 0.9 the argument warning
    # existed in leastsq(), but version 0.9 and later converted the function
    # to issue the related messages as Python warnings class messages.  Thus,
    # for this leastsq() call to act the same for both versions the old version
    # is assumed and if it crashes, the except block performs the new version
    # and suppresses the warning messages with warnings.simplefilter('ignore')
    try:
        (Bg, Peak, Cx, Cy, Wx, Wy, Theta),success = minpack.leastsq(errorfunction, initialParams, maxfev=100, warning=False)
    except:
        import warnings
        warnings.simplefilter('ignore')
        (Bg, Peak, Cx, Cy, Wx, Wy, Theta),success = minpack.leastsq(errorfunction, initialParams, maxfev=100)
...
def moffat2d(stampArray,pixelscale,initialParams,stampPars,outFile, debug, frame):
    ...
    try:
        ...
        import warnings
        warnings.simplefilter('ignore')
        (Bg, Peak, Cx, Cy, Wx, Wy, Theta, Beta),success = minpack.leastsq(errorfunction,initialParams, maxfev=100, col_deriv=1, factor=150)
