...
try:
    ...
    parser.add_option("-p", "--param", dest="userparam", default=None,
                      ...
                      "SETTYPE:primitivename:paramname=val', sets the parameter "
                      "such that it applies only when the current reduction type "
                      "(type of current reference image) is 'SETTYPE' and the "
    ...
    def command_line():
        ...
        try:
            ...
            log.error("NO INPUT FILE or DATATYPE specified")
    ...
    for infiles in allinputs: #for dealing with multiple sets of files.
        ...
        try:
            ...
            if infiles:
                for infs in infiles:
                    infsclass = type(infs)
                    if infsclass == AstroData or issubclass(infsclass, generaldata.GeneralData):
                        ...
                        raise "Type of data not understood.  Not expected to happen"
            ...
            if infiles:
                ...
                log.error("Recipe not found for NO DATA. See -r option.")
